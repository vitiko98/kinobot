...
class Kinobase:
    ...
    def _execute_sql(self, sql: str, params: tuple):
        logger.debug("Database path: %s", self._database)
        with sqlite3.connect(self._database) as conn:
            conn.execute("PRAGMA journal_mode=WAL")
            conn.set_trace_callback(logger.debug)
    ...
    def _db_command_to_dict(
        ...
        logger.debug("Database path: %s", self._database)
        return sql_to_dict(self._database, sql, params)
    ...
    def _update(self, id_):
        command = (
            f"update {self.table} set {'=?, '.join(self._insertables)}=? where id=?"
        )
    ...
    def _get_insert_command(self) -> str:
        ...
        return f"insert or ignore into {self.table} ({columns}) values ({placeholders})"

    def _get_sqlite_tuple(self) -> tuple:
        return tuple(getattr(self, attr) for attr in self._insertables)
